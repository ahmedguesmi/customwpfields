<?php
  ...
  */
namespace Ahmedguesmi\Customwpfields\Fields\Customizer;
use WP_Customize_Control as WP_Customize_Control;
use Ahmedguesmi\Customwpfields\Fields\Typography as TypographyField;
use Ahmedguesmi\Customwpfields\Fields\Dimension as DimensionField;
            ...
            <select class="wpcf-typography-fonts" <?php $this->link('font'); ?> >
                ...
                <?php foreach( $configurations['properties']['fonts'] as $fontspace => $types ) { ?>
                    ...
                    <?php foreach( $types as $key => $font ) { ?>         
                        <?php $display = isset($font['example']) ? $font['example'] : CUSTOMWPFIELDS_ASSETS_URL . 'img/' . $key . '.png'; ?>
