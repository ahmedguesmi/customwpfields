...
class Typography implements Field {
    ...
    public static function render( array $field = [] ): void {
        ...
        if( apply_filters('customwpfields_select_field_js', true) && ! wp_script_is('select2-js', 'enqueued') )
                ...
                <select class="wpcf-typography-fonts" name="<?php echo esc_attr($field['name']); ?>[font]" id="<?php echo esc_attr($field['id']); ?>-font" >
                    ...
                    <?php foreach( $configurations['properties']['fonts'] as $fontspace => $types ) { ?>
                        ...
                        <?php foreach( $types as $key => $font ) { ?>         
                            <?php $display = isset($font['example']) ? esc_url($font['example']) : CUSTOMWPFIELDS_ASSETS_URL . 'img/' . $key . '.png'; // Allows for custom fonts ?> 
    ...
    public static function configurations(): array {
        ...
        return apply_filters( 'customwpfields_typography_config', $configurations );
