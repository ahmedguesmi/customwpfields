...
trait Validate {
    ...
    public static function add_error_message( array $frame_options = [], string $type = 'update' ): void {
        ...
        switch( $type ) {
            case 'reset':
                add_settings_error( $frame_options['id'], 'customwpfields-notification', $messages['reset'], 'info' );
                break;
            case 'restore':
                add_settings_error( $frame_options['id'], 'customwpfields-notification', $messages['restore'], 'info' );
                break;  
            case 'update':
                add_settings_error( $frame_options['id'], 'customwpfields-notification', $messages['update'], 'success' );
                break;                               
            case 'import':  
                add_settings_error( $frame_options['id'], 'customwpfields-notification', $messages['import'], 'info' );
                break;  
        }
    ...
    public function format( array $frame_options, array $input, string $type = '' ): array {
        ...
        $currentTab = strip_tags( $input['customwpfields_section_' . $frame_options['id']] );
        
        // Sets the transient for the current section
        set_transient('customwpfields_current_section_' . $frame_options['id'], $currentTab, 10); 
    ...
    private static function sanitize_field( $input, array $field ) {
        ...
        return apply_filters( 'customwpfields_sanitized_value', $return_value, $field_value, $field );
