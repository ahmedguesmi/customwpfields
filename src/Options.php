...
class Options { 
    ...
    public function render_page( $args ) {
        ...
        // Save Button
        ob_start();
        submit_button( $labels['save'], 'primary button-hero customwpfields-save', $page_ID . '_save', false );
        $frame->save_button             = ob_get_clean();

        ob_start();
        submit_button( $labels['save'], 'primary button-hero customwpfields-save', $page_ID . '_save_bottom', false );
        $frame->save_button_bottom      = ob_get_clean();


        // Restore Button
        ob_start();
        submit_button( $labels['restore'], 'delete button-hero customwpfields-reset-section', $page_ID . '_restore', false );
        $frame->restore_button          = ob_get_clean();

        // Restore Button
        ob_start();
        submit_button( $labels['restore'], 'delete button-hero customwpfields-reset-section', $page_ID . '_restore_bottom', false );
        $frame->restore_button_bottom   = ob_get_clean();        

 
        // Reset Button
        ob_start();
        submit_button( $labels['reset'], 'delete button-hero customwpfields-reset', $page_ID . '_reset', false );
