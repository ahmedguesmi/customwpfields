 ...
 *  Author URI:  https://www.ahmedguesmi.com
 *  Domain Path: /languages
 *  Text Domain: customwpfields
 */
...
class Framework extends Base {
    ...
    protected function initialize(): void {
        ...
        // Define Constants
        defined( 'CUSTOMWPFIELDS_ASSETS_URL' ) or define( 'CUSTOMWPFIELDS_ASSETS_URL', content_url() . $folder . '/assets/' );
        defined( 'CUSTOMWPFIELDS_PATH' ) or define( 'CUSTOMWPFIELDS_PATH', plugin_dir_path( __FILE__ ) );
    ...
    private function add_configurations(): void {
        ...
        // Back-end assets
        if( is_admin() || is_customize_preview() ) {  
            require_once( CUSTOMWPFIELDS_PATH . 'config/scripts.php' );
            require_once( CUSTOMWPFIELDS_PATH . 'config/styles.php' );
            require_once( CUSTOMWPFIELDS_PATH . 'config/icons.php' );     
                    
            $this->scripts              = $scripts;
            $this->styles               = $styles;        
            self::$icons                = apply_filters( 'customwpfields_icons', $icons );
        }
        
        // Fonts are also used in front-end styling
        require_once( CUSTOMWPFIELDS_PATH . 'config/fonts.php' );
        self::$fonts                    = apply_filters( 'customwpfields_fonts', $fonts ); 
                
        if( is_admin() || is_customize_preview() ) { 
            
            // Setup the supported datatypes
            $this->types                = apply_filters( 'customwpfields_frames',  $this->types );
        
            // Adds filterable data for the various types.
            foreach( $this->types as $type ) {
                $this->frames[$type]    = apply_filters( 'customwpfields_frame_' . $type, isset($this->frames[$type]) ? $this->frames[$type] : [] );
            }
    ...
    private function frame(): void {
        ...
        foreach( $this->frames as $frame => $options_groups ) {
            ...
            foreach( $options_groups as $group ) {
                $class    = 'Ahmedguesmi\Customwpfields\\' . ucfirst( $frame );
